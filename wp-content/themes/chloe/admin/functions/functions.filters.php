 ...
 * 
 * @since 1.4.0
 * @param $chloe_data Options array
 * @return array
 */
function of_filter_save_media_upload($chloe_data) {

    if(!is_array($chloe_data)) return $chloe_data;
    
    foreach ($chloe_data as $key => $value) {
        if (is_string($value)) {
            $chloe_data[$key] = str_replace(
                array(
                    site_url('', 'http'),
                    site_url('', 'https'),
                ),
 ...
 * @return array
 */
function of_filter_load_media_upload($chloe_data) {
