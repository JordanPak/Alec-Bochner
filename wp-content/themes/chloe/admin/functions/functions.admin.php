 ...
 * @return array
 */
function of_get_options($key = null, $chloe_data = null) {
	global $smof_data;

	do_action('of_get_options_before', array(
		'key'=>$key, 'data'=>$chloe_data
	));
	if ($key != null) { // Get one specific value
		$chloe_data = get_theme_mod($key, $chloe_data);
	} else { // Get all values
		$chloe_data = get_theme_mods();	
	}
	$chloe_data = apply_filters('of_options_after_load', $chloe_data);
	if ($key == null) {
		$smof_data = $chloe_data;
	} else {
		$smof_data[$key] = $chloe_data;
	}
 ...
 *
 * @param $chloe_data Options array to save
 * @author Jonah Dahlquist
 * @since 1.4.0
 * @uses update_option()
 * @return void
...
function of_save_options($chloe_data, $key = null) {
	global $smof_data;
    if (empty($chloe_data))
        return;	
	...
	$chloe_data = apply_filters('of_options_before_save', $chloe_data);
	if ($key != null) { // Update one specific value
		if ($key == BACKUPS) {
			unset($chloe_data['smof_init']); // Don't want to change this.
		}
		set_theme_mod($key, $chloe_data);
	} else { // Update all values in $chloe_data
		foreach ( $chloe_data as $k=>$v ) {
