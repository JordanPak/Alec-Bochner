...
class chloe_sidebar_generator {
	
	function chloe_sidebar_generator(){
		add_action('init',array('chloe_sidebar_generator','chloe_init'));
		add_action('admin_menu',array('chloe_sidebar_generator','chloe_admin_menu'));
		add_action('admin_enqueue_scripts', array('chloe_sidebar_generator','chloe_admin_scripts'));
		add_action('wp_ajax_add_sidebar', array('chloe_sidebar_generator','chloe_add_sidebar') );
		add_action('wp_ajax_remove_sidebar', array('chloe_sidebar_generator','chloe_remove_sidebar') );
			
		//edit posts/pages
		add_action('edit_form_advanced', array('chloe_sidebar_generator', 'chloe_edit_form'));
		add_action('edit_page_form', array('chloe_sidebar_generator', 'chloe_edit_form'));
		
		//save posts/pages
		add_action('edit_post', array('chloe_sidebar_generator', 'chloe_save_form'));
		add_action('publish_post', array('chloe_sidebar_generator', 'chloe_save_form'));
		add_action('save_post', array('chloe_sidebar_generator', 'chloe_save_form'));
		add_action('edit_page_form', array('chloe_sidebar_generator', 'chloe_save_form'));

	}
	
	public static function chloe_init(){
		//go through each sidebar and register it
	    $sidebars = self::chloe_get_sidebars();
	    

	    if(is_array($sidebars)){
			foreach($sidebars as $sidebar){
				$sidebar_class = self::chloe_name_to_class($sidebar);
	...
	public static function chloe_admin_scripts(){
		wp_print_scripts( array( 'sack' ));
		?>
			<script>
				var ch_dir = '<?php echo get_stylesheet_directory_uri() ?>';
	...
	public static function chloe_add_sidebar(){
		$sidebars = self::chloe_get_sidebars();
		$name = str_replace(array("\n","\r","\t"),'',$_POST['sidebar_name']);
		$id = self::chloe_name_to_class($name);
		if(isset($sidebars[$id])){
			die("alert('Sidebar already exists, please use a different name.')");
		}
		
		$sidebars[$id] = $name;
		self::chloe_update_sidebars($sidebars);
	...
	public static function chloe_remove_sidebar(){
		$sidebars = chloe_sidebar_generator::chloe_get_sidebars();
		$name = str_replace(array("\n","\r","\t"),'',$_POST['sidebar_name']);
		$id = chloe_sidebar_generator::chloe_name_to_class($name);
		if(!isset($sidebars[$id])){
			die("alert('Sidebar does not exist.')");
		}
		$row_number = $_POST['row_number'];
		unset($sidebars[$id]);
		chloe_sidebar_generator::chloe_update_sidebars($sidebars);
		$js = "
			var tbl = document.getElementById('sbg_table');
	...
	public static function chloe_admin_menu(){
		add_theme_page('Sidebar Generator', 'Sidebar Generator', 'edit_theme_options', 'sidebar-generator', array('chloe_sidebar_generator','chloe_admin_page'));
	}
	
	
	public static function chloe_admin_page(){
		?>
		<script>
		...
		<div class="wrap">
			...
			<div class="add_sidebar">
				<a class="button button-primary" href="javascript:void(0);" onclick="return add_sidebar_link()" title="Add a sidebar">+ Add Sidebar</a>
			</div>
			<br />
			<table class="widefat page" id="sbg_table" style="width:600px;">
				...
				<tr class="<?php echo $alt?>">
					...
					<td><?php echo chloe_sidebar_generator::chloe_name_to_class($sidebar); ?></td>
	...
	*/
	public static function chloe_save_form($post_id){
		...
		if(!empty($is_saving)){
			...
			add_post_meta($post_id, 'sbg_selected_sidebar', $_POST['chloe_sidebar_generator']);
			add_post_meta($post_id, 'sbg_selected_sidebar_replacement', $_POST['chloe_sidebar_generator_replacement']);
		}		
	}
	
	public static function chloe_edit_form(){
	    global $post;
	    $post_id = $post;
			...
			<div class="inside">
				<div class="sbg_container">
					...
					<p>
						...
						<strong>Note:</strong> You must first create the sidebar under Appearance > Sidebar Generator. 
					...
					//var_dump($wp_registered_sidebars);		
						for($i=0;$i<1;$i++){ ?>
							<li>
							<select name="chloe_sidebar_generator[<?php echo $i?>]" style="display: none;">
							...
							<select name="chloe_sidebar_generator_replacement[<?php echo $i?>]">
	...
	*/
	public static function chloe_get_sidebar($name="0"){
		if(!is_singular()){
			if($name != "0"){
	...
	*/
	public static function chloe_update_sidebars($sidebar_array){
	...
	*/
	public static function chloe_get_sidebars(){
		$sidebars = get_option('sbg_sidebars');
		return $sidebars;
	}
	public static function chloe_name_to_class($name){
		$class = str_replace(array(' ',',','.','"',"'",'/',"\\",'+','=',')','(','*','&','^','%','$','#','@','!','~','`','<','>','?','[',']','{','}','|',':',),'',$name);
		return $class;
	}
	
}
$sbg = new chloe_sidebar_generator;

function chloe_generated_dynamic_sidebar($name='0'){
	chloe_sidebar_generator::chloe_get_sidebar($name);	
	return true;
}
?>